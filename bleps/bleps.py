...
class Target:
    ...
    def predict_eclipses(self, observer_object, search_start, search_end, alt_limits=(30, 80), strategy='split'):
        ...
        if strategy == 'split':
            ...
            types = [str, int, str, str, str, str, str, str] + [str, float, float] * 2
        elif strategy == 'full':
            ...
            types = [str, int, str, str, str, str, str] + [str, float, float] * 3
        elif strategy == 'any':
            colnames = ["Name", "EclType", "RA", "Dec", "Moon", "UTC_sunset", "UTC_sunrise",
                        "UTC_start", "Alt_start", "Az_start", "OK_start", "UTC_mid", "Alt_mid", "Az_mid", "OK_mid",
                        "UTC_end", "Alt_end", "Az_end", "OK_end"]
            types = [str, int, str, str, str, str, str] + [str, float, float, bool] * 3
        else:
...
def gcal_format(table, observatory):
    """
    ...
    :param observatory: Dictionary with information on observatory. Keys: name, lat, lon, elev; coords given as floats
    :type table: astropy.table.Table
    :return: Reformatted table ready to save as .csv
    """
    names = ['Subject', 'Start Date', 'Start Time', 'End Date', 'End Time', 'All Day Event', 'Location', 'Description']
    ...
    for row in table:
        ...
        all_day = 'FALSE'
        loc = f"{observatory['lat']}, {observatory['lon']}"
        description = (f"This is an {io} starting at {t_start} UTC. "
                       f"Moon: {row['Moon']}, sunset: {row['UTC_sunset'][11:16]} UTC, "
                       f"sunrise: {row['UTC_sunrise'][11:16]} UTC. {altaz_info}")

        gcal.add_row([subject, d_start, t_start, d_end, t_end, all_day, loc, description])
